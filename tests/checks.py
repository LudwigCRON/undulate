...
def check_similarity(ref, img, black_white: bool = False):
    ...
    return compare_ssim(
        ref, img, win_size=9, data_range=img.max() - img.min(), multichannel=True
    )
...
def normalize_img(
    ...
    if path.endswith("svg") or path.endswith("eps") or path.endswith("pdf"):
        ...
        if is_ref:
            os.popen(
                "convert -density 300 -trim -resize %dx%d -background white -compose Copy -gravity center %s %s"
                % (width, height, path, filename)
            ).read()
        else:
            os.popen(
                "convert -density 300 -trim -resize %dx%d -background white -compose Copy -gravity center -extent %dx%d %s %s"
                % (width, height, width, height, path, filename)
            ).read()
