...
function renderArcs (root, source, index, top, lane) {
    var gg,
        ...
        text,
        Stack = [],
        Edge = {words: [], from: 0, shape: '', to: 0, label: '', color:'#0041c4'},
        Events = {},
        pos,
        eventname,
        // labeltext,
    ...
    function t1 () {
        if (from && to) {
            ...
            gmark.setAttribute('style', 'fill:none;stroke:'+Edge.color+';stroke-width:1');
            gg.insertBefore(gmark, null);
        }
    }

    function getEdgeStyle (index) {
        if (top.style) {
            for(let e of top.style) {
                if(e.hasOwnProperty('edge') && e.edge == index) return e
            }
        }
        return null
    }
    ...
    if (source) {
        ...
        if (top.edge) {
            for (i in top.edge) {
                ...
                Edge.color = (EdgeStyle != null && EdgeStyle.hasOwnProperty('color')) ? EdgeStyle.color : "#0041c4";
                console.log(i, Edge.from, Edge.to)
                ...
                if (from && to) {
                    ...
                    case '->' : {
                        gmark.setAttribute('style', 'marker-end:url(#arrowhead);stroke:'+Edge.color+';stroke-width:1;fill:none');
                        break;
                    }
                    case '~>' : {
                        gmark.setAttribute('style', 'marker-end:url(#arrowhead);stroke:'+Edge.color+';stroke-width:1;fill:none');
                    ...
                    case '<->' : {
                        gmark.setAttribute('style', 'marker-end:url(#arrowhead);marker-start:url(#arrowtail);stroke:'+Edge.color+';stroke-width:1;fill:none');
                        break;
                    }
                    case '<~>' : {
                        gmark.setAttribute('style', 'marker-end:url(#arrowhead);marker-start:url(#arrowtail);stroke:'+Edge.color+';stroke-width:1;fill:none');
                ...
                if (EdgeStyle != null) {
                    let dlx = (EdgeStyle.hasOwnProperty('dx') ? EdgeStyle.dx : 0);
                    let dly = (EdgeStyle.hasOwnProperty('dy') ? EdgeStyle.dy : 0);
                    label.setAttribute('transform', 'translate('+dlx+', '+dly+')')
                    underlabel.setAttribute('transform', 'translate('+dlx+', '+dly+')')
                }
            }
        }
        if(!top.hasOwnProperty('hide_edge_markers') || !top.hide_edge_markers) {
            for (k in Events) {
                if (k === k.toLowerCase()) {
                    if (Events[k].x > 0) {
                        underlabel = jsonmlParse(['rect',
